...
class Disease:
    ...
    def get_suggestion(self, text):
        """
        专门用于前端搜索建议的轻量级查询
        特点：去掉了标签限制(防止搜不到)，加了 limit 10 (防止数据太多卡死)
        """
        if not text:
            return []

        # 使用 contains 语法，比正则更快，且不限制标签
        cql = f"match(n) where n.name contains '{text}' return n.name as name limit 10"

        suggestions = []
        try:
            for item in self.graph.run(cql).data():
